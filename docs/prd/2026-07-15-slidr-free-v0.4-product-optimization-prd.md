...
设置窗口改为四个稳定入口，建议使用原生 macOS 侧栏或顶部标签，窗口默认尺寸约 640 × 560，并允许缩放：

1. **概览**：整体健康状态、启用应用、登录时启动、快速修复、最近识别反馈；
2. **边缘手势**：触控板示意、左/右/上边缘动作、边缘宽度、测试入口；
3. **中键点击**：总开关、轻点、手指数、冲突提示、测试入口；
4. **诊断**：权限、框架、设备、触摸监视器、物理点击监听、最近失败、版本与复制诊断。
...
6. 开发包明确标注为 ad-hoc，验证版本、构建号、Bundle ID、许可证和归档内容。
7. README 和诊断页保留单默认设备、私有框架、ad-hoc 权限恢复等已知限制；其他 macOS 和外接触控板由社区按需验证，不阻塞 v0.4。
...
v0.4.0 一次性交付 M1、M2 和上述轻量 M3；不为了缺失的 Apple 开发者资源推迟产品体验，也不增加新的手势类型。
