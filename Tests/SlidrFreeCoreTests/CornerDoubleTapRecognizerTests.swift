...
final class CornerDoubleTapRecognizerTests: XCTestCase {
    ...
    func testLateOrLongSecondTapDoesNotTrigger() {
        ...
        XCTAssertNil(late.process(frame([touch(2, x: 0.05, y: 0.95)], 1.60), cornerWidthPercent: 0.10))
        XCTAssertNil(late.process(frame([], 1.70), cornerWidthPercent: 0.10))
        ...
        XCTAssertNil(long.process(frame([], 2.51), cornerWidthPercent: 0.10))
    }
    ...
    func testQualifyingCornerDoubleTapCannotAlsoEmitMinimumDistanceEdgeStep() {
        ...
        XCTAssertNil(recognizer.process(frame([touch(1, x: 0.05, y: 0.05)], 1.00)))
        XCTAssertNil(recognizer.process(frame([touch(1, x: 0.05, y: 0.064)], 1.05)))
